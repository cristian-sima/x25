...
const initialState: CurrentState = Immutable.Map({
  ...
  fetchCompanyPending = () => initialState.set("fetching", true),
  fetchCompanyRejected = (state: any, {
    payload: {
      error,
    },
  }) => state.merge({
  ...
  fetchCompanyFulfilled = (state: any, {
    payload,
  }) => state.mergeDeep({
    fetched  : true,
    fetching : false,
  ...
  modifyCompany = (state: any, {
    payload,
  }) => state.set("company", payload),
  clearInfoIfCurrentCompany = (state: any, {
    payload,
  }) => {
    const companyID = String(state.getIn(["company", "ID"])),
  ...
  getFetching = (state: State) => state.getIn([reducerKey, "fetching"]) || false,
  getFetched = (state: State) => state.getIn([reducerKey, "fetched"]) || false,
  getError = (state: State) => state.getIn([reducerKey, "error"]) || noError,
  getCurrentAccountFetched = (state: State) => state.getIn(["account", "fetched"]) || false,

  checkForNoErrors = (error) => error !== noError,

  getCurrentCompany = (state: State): any => state.getIn([reducerKey, "company"]) || Immutable.Map(),
  getCurrentCompanyIsFetched = createSelector(getFetching, getFetched, getError, (isFetching, isFetched, error) => !isFetching && isFetched && error === noError),
  getCurrentCompanyIsFetching = createSelector(getFetching, getError, (isFetching, error) => isFetching && error === noError),
  getCurrentCompanyHasError = createSelector(getError, checkForNoErrors),
  getCurrentCompanyShouldFetch = createSelector(getCurrentAccountFetched, getCurrentCompanyIsFetched, getCurrentCompanyHasError, getFetching, getCurrentCompany, (accountFetched, isFetched, hasError, isFetching, company) => (rawID: string) => {
    const hasIDChanged = () => {
      const id = company.get("ID"),
        newID = Number(rawID);
  ...
  getCompanyModules = createSelector(getCurrentCompany, (company) => company.get("Modules") || ""),
