...
class PaymentWrap extends React.Component<PaymentWrapPropTypes, PaymentWrapStateTypes> {
  ...
  constructor (props: PaymentWrapPropTypes) {
    ...
    this.createPayment = (formData: any) => {
      const {
        ...
        Details = getDetails(this.props.application, { ...data,
          companyID,
        }),
        toSend = {
          CompanyID: companyID,
      ...
      return addPaymentRequest(toSend).then((response: any) => {
        if (response.Error === "") {
          this.setState({
  ...
  render () {
    ...
    if (this.state.show3rdServiceForm) {
      return <MobilpayForm data={this.state.data} envKey={this.state.envKey} url={this.props.url} />;
    }
