...
type FormPropTypes = {
  ...
  readonly handleSubmit: (formData: any) => Promise<any>;
};
type DoneFormPropTypes = {
  readonly details: string;
  readonly showUserResponse: () => void;
};
...
import { extractErrorsFromCheckers, validateString, delay, ReduxFormSubmissionError } from "../../utility";
...
const TransferNumber = validateString({
  ...
  validate = extractErrorsFromCheckers({
    TransferNumber,
  }),
  selector = formValueSelector("ADD_CAR_FORM"),
...
class TheForm extends React.Component<FormPropTypes> {
  ...
  render () {
    ...
    return (<form autoComplete="off" className="mt-4" onSubmit={this.handleSubmitForm}>
      ...
      <div className="container">
        <div className="row">
          <div className="col-md">
            <Field autoFocus component={FocusTemplate} forwardRef label="Număr tranzacție" name="TransferNumber" onRegisterRef={this.handleRegisterRef} placeholder="ex. 238747324" />
          </div>
        </div>
      </div>
      <div className="text-center mb-4">
        {submitting ? <LoadingMessage sm /> : (<button aria-label="Trimite" className="btn btn-primary" disabled={pristine || submitting} type="submit">
          {"Trimite"}
                                               </button>)}
      </div>
    </form>);
...
class DoneForm extends React.Component<DoneFormPropTypes> {
  ...
  constructor () {
    ...
    this.handleSubmit = (formData: any) => {
      ...
      return confirmBankTransferRequest(data).then((response: any) => {
        if (response.Error === "") {
          showUserResponse();
        } else {
