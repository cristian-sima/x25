...
export const isValidCNP = (rawValue: string): boolean => {
  ...
  type PositionPropTypes = Record<string, number>;
  ...
  const decimalSystem = 10,
    ...
    getCNP = (raw: string): CNP => {
      ...
      const hashTable = [2, 7, 9, 1, 4, 6, 3, 5, 8, 2, 7, 9],
        ...
        reduceHash = (rawHash: number): number => {
          const nrOfDigits = 11,
      ...
      const controlDigit = parseInt(raw.charAt(position.controlDigit), decimalSystem);
        ...
        isValidDate = () => {
          const getYear = (): number => {
              const computeYear = (): number => {
                  ...
                  let year = getCNPDigit(yearDecades) * 10 + getCNPDigit(yearUnits);
