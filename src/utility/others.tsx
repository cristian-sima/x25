...
export const noError = "",
  rowsPerLoad = 50,
  nothingFetched = -1;
export const removeID = (payload: ImmutableMap<string, any>) => (list: ImmutableList<string>) => // $FlowFixMe
  list.delete(list.findIndex((current: string) => current === String(payload.get("ID"))));
export const addID = (payload: ImmutableMap<string, any>) => (list: ImmutableList<string>) => list.push(String(payload.get("ID")));
...
export const showCheck = (value: boolean) => value ? <i className="fa fa-check text-success" /> : null;
